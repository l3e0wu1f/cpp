...
//
//
//  Created by Josh Lewis on 10/26/17.
//
//

// Question 1: YES. Checks if day is valid based on month entered via switch statement, then checks if month is between 1 and 12 and year is later than 1582.
// Question 2: YES. When the date is not valid, checking the above conditions, an error message is printed.
// Question 3: NO. My implementation was not the same. I did not think to make the default case in the switch statement return a value of -1.
...
// precondition: year is a legal year, and fileName is the name of output file
    ...
    do
    {
        ...
        switch(choice)
        {
            ...
            case '4': // print out the calendar of specified year to yourname.txt file
                get_input(year);
                if(print_calendar(year, "joshlewis.txt")){ // prints calendar via custom output file stream to file
                    cout << "The calendar of year " << year << " has been printed to joshlewis.txt\n";
                }
                else{
            ...
            case '5': // quit the program.
                cout<<"\n\tThank you for using Josh's calendar program!\n\n";
                break;
...
{
    if(!isValid(year)){
        cout << "Error: The year is not valid!";
        return false;
    }
    ofstream localOut;
    localOut.open(fileName);
    ...
    for(int i = 1; i <= 12; i++) // print out 12 month calendar
    {
        print_calendar(i, year, localOut); // print out ith month
        localOut << "\n\n";
    }
